...
static struct platform_device gfifo_platform_device = {
	.name = "gfifo",
	.id = -1,
};
...
{
	return platform_device_register(&gfifo_platform_device);
}
module_init(gfifo_device_init);

static void __exit gfifo_device_exit(void)
{
	platform_device_unregister(&gfifo_platform_device);
}
